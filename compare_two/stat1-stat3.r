...
args = sys$cmd$parse(
    ...
    opt('p', 'plotfile', 'pdf', 'stat1-stat3.pdf')
)

lookup = c(
    "rev stat1 vs stat3" = "diff",
    "BT549 STAT3 KO Reversine vs DMSO" = "y",
    "stat1 ko rev vs dmso" = "x"
)
...
arts = c("more stat3-driven", "more stat1-driven")
arrws = sres2 %>%
    filter(label %in% arr) %>%
