...
args = sys$cmd$parse(
    ...
    opt('p', 'plotfile', 'pdf', '2-rev_stat1_wt.pdf')
)
...
cmp_sets = cmp_sets %>%
    ...
    mutate(log2FoldChange = ifelse(cond == "y", log2FoldChange - (1-0.9) * wt_lfc, log2FoldChange)) %>%
    select(-wt_lfc)
...
show_lab = c(
    ...
    "Inflammatory Response", "Epithelial Mesenchymal Transition",
...
sres2 = cmp_sets %>%
    ...
    mutate(label = ifelse(adj.p < 1e-10 | label %in% show_lab, label, NA),
           min_p = cut(adj.p, breaks=c(0, 1e-10, 0.01, Inf), labels=c("<1e-10", "<0.01", "n.s.")))

arr = c("Interferon Gamma Response", "Interferon Alpha Response", "KRAS Signaling Dn",
        "IL-6/JAK/STAT3 Signaling", "TNF-alpha Signaling via NF-kB", "STAT3")
