...
args = sys$cmd$parse(
    ...
    opt('p', 'plotfile', 'pdf', 'x-wt_relb.pdf')
)

lookup = c(
    "RelB KO Reversine vs WT Reversine" = "diff",
    "BT549 RelB KO Reversine vs DMSO" = "y",
...
cmp_sets = res %>%
    select(cond=comparison, genes) %>%
    tidyr::gather("collection", "stats", -cond) %>%
    tidyr::unnest()
cmp_sets = cmp_sets %>%
    left_join(cmp_sets %>% filter(cond == "x") %>% select(ensembl_gene_id, wt_lfc=log2FoldChange)) %>%
    mutate(log2FoldChange = ifelse(cond == "y",
                log2FoldChange - (1-0.9) * (wt_lfc - log2FoldChange), log2FoldChange)) %>%
    select(-wt_lfc)
...
sres2 = cmp_sets %>%
    filter(cond != "diff") %>%
    select(cond, collection, ensembl_gene_id, padj) %>%
    tidyr::pivot_wider(names_from=c(cond), values_from=c(padj)) #%>%
#    left_join(cmp_sets %>% filter(cond == "y") %>% select(ensembl_gene_id, fdr_y=padj)) %>%
#    left_join(cmp_sets %>% filter(cond == "diff") %>% select(ensembl_gene_id, fdr_diff=padj)) %>%
#    left_join(cmp_sets %>% select(ensembl_gene_id, label) %>% distinct()) %>%
...
ggplot(sres2, aes(x=x, y=y)) +
    ...
    geom_point(aes(size=1, fill=collection, alpha=min_p, shape=p_src), shape=21) +
