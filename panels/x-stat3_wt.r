...
args = sys$cmd$parse(
    ...
    opt('p', 'plotfile', 'pdf', 'x-stat3_wt.pdf')
)

eset = readRDS(args$infile)
eset = eset[, eset$genotype %in% c("stat3", "wt") &
              eset$treatment %in% c("dmso", "rev") & eset$time == "48"]

eset$genotype = droplevels(eset$genotype)
eset$treatment = droplevels(eset$treatment)
eset$stat3_dep = (eset$genotype == "wt" & eset$treatment == "rev") + 0
eset$stat3_indep = (eset$genotype == "stat3" & eset$treatment == "rev") + 0
design(eset) = ~ genotype + stat3_dep + stat3_indep #genotype:treatment
model.matrix(design(eset), colData(eset))

mod = DESeq2::DESeq(eset)
resultsNames(mod)
res = list(stat3_dep = DESeq2::results(mod, name="stat3_dep"),
           stat3_indep = DESeq2::results(mod, name="stat3_indep")) %>%
...
de_genes = list(stat3_dep = res$stat3_dep$ensembl_gene_id[res$stat3_dep$padj < 0.1],
                stat3_indep = res$stat3_indep$ensembl_gene_id[res$stat3_indep$padj < 0.1])
#plt$venn(de_genes)
...
sres = tidyr::crossing(tibble(cond=c("stat3_dep", "stat3_indep")), tibble(set=snames)) %>%
    rowwise() %>%
...
sres2 = sres %>%
    ...
    mutate(label = ifelse(label %in% show_lab, label, NA),
           min_p = apply(cbind(adj.p_stat3_dep, adj.p_stat3_indep), 1, function(x) min(x, na.rm=TRUE)),
           min_p = cut(min_p, breaks=c(0, 1e-10, 0.01, Inf), labels=c("<1e-10", "<0.01", "n.s.")))

cairo_pdf(args$plotfile, 9, 7)
ggplot(sres2, aes(x=estimate_stat3_dep, y=estimate_stat3_indep)) +
    ...
    geom_text(data=data.frame(x=0.5, y=0.5, txt="more stat3-independent ⇖    ⇘ more stat3-dependent  "),
